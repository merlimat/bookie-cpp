...
void BookieCodecV2::read(Context* ctx, IOBufPtr buf) {
    if (!buf) {
        return;
    }
    ...
    case BookieOperation::AddEntry:
        ...
        if (reader.totalLength() < addRequestSize) {
            LOG_WARN("Invalid add entry request size: " << reader.totalLength() << " -- expecting at least: " << addRequestSize);
            ctx->fireClose();
            return;
        }
    ...
    case BookieOperation::ReadEntry: {
        ...
        if (reader.totalLength() < readRequestSize) {
            LOG_WARN("Invalid read entry request size: " << reader.totalLength() << " -- expecting: " << readRequestSize);
            ctx->fireClose();
            return;
        }
...
Future<Unit> BookieCodecV2::write(Context* ctx, Response response) {
